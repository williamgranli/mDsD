...
public class MakeBookinUseCaseTest {
	...
	public void setUp() throws Exception {
		...
		rh.createBedRoom(105, true, 899, rt[1], "For the lone wolf", 1);
		rh.createBedRoom(106, true, 899, rt[1], "For the lone wolf", 1);
	}
	...
	public void makeBookingSuccessFlow() {
		 ...
		 */
		String ref1 = ib.makeBooking(rt[1], ad, dd, "", "", "", "", "", "", 0, 0); // NOTE: Not through IBooking
		 ...
		 */
		// NOTE: Payment component not accessible if not on Chalmers network. Cannot VPN atm.
		//if(ip.validateCC(custCC, custCVC, custExpMonth, custExpYear, custFName, custLName)) {
			bh.findBooking(ref1).addPaymentDetails(custFName, custLName, custAddr, custCC, custCVC, custExpMonth, custExpYear);	
		//}
	...
	public void inputFaultyCCDetailsThreeTimes() {
		...
		int i = 8;
		while (!correct) {
			if (custCC.substring(0, i).length() == 12 && custCVC.length() == 3 && custExpMonth < 13 && custExpMonth > 0 && custExpYear < 10000 && custExpYear > 0) {
